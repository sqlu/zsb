...
/*
 * Command Hide
 * This command allows the selfbot user to hide a channel
 */

const hideCommand: SlashCommand = {
  name: "hide",
  description: "Allows you to hide a channel.",
  description_localizations: {
    fr: "Permet de masquer un salon.",
  },
  options: [
    {
      name: "channel",
      description: "The channel that you wish to hide.",
      type: 7,
      description_localizations: {
        fr: "Le salon que vous souhaitez masquer.",
      },
      channel_types: [0],
    },
  ],
      ...
      await interaction.editReply({
        content:
          interaction.locale === "fr"
            ...
            : `The channel ${channel} has been successfully hidden.`,
      });
    } catch (err) {
      console.log(err);
      interaction.editReply({
        content:
          interaction.locale === "fr"
            ? `Vous ne possédez pas les permissions nécessaires pour masquer un salon !`
            : `You do not have the necessary permissions to hide a channel!`,
      });
    }
